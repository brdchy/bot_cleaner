...
def load_patterns_from_csv(filename):
    ...
    try:
        ...
        print(f"Файл {filename} не найден. Создаем новый список паттернов.")
    return ad_patterns

ad_patterns = load_patterns_from_csv('ad_patterns.csv')
...
test_messages = [
    ...
    "Наконец то, описание вакансии для тех кому интересен хороший дополнительный заработок в свободное время - пишите в лс. От 18 лет частичная занятость удалённая работа высокий доход",
    "Ищем сотрудников на должность перевозчика - Работа полностью легальная (НЕ НАРКОТИКИ) - ⁠Работаем со всеми городами РФ - ⁠Оплата по выполнению заказа день в день Доход от 80.000₽ в месяц"
]
